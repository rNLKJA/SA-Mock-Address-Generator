...
def main():
    ...
    council_addr = lookup.generate_random_address(
        distribution_type="council", distribution_value="CITY OF ADELAIDE"
    )
    ...
    city_addr = lookup.generate_random_address(
        distribution_type="remoteness", distribution_value="Major Cities of Australia"
    )
    ...
    socio_addr = lookup.generate_random_address(
        distribution_type="socioeconomic", distribution_value="5"
    )
    ...
    print(
        f"   Suburbs ({len(options['suburbs'])}): {', '.join(options['suburbs'][:5])}..."
    )
