...
class SAAddressLookup:
    ...
    def __init__(
        ...
        data_file: str = "data/sa_suburbs_data.csv",
    ):
        """
        ...
        """
        self.mapbox_api_key = mapbox_api_key or os.getenv(
            "MAPBOX_API_KEY"
        )  # Fallback to env var
        ...
        for attempt in range(self.MAX_VALIDATION_RETRIES):
            try:
                ...
                if attempt == self.MAX_VALIDATION_RETRIES - 1:
                    print(
                        f"Failed to lookup address after {self.MAX_VALIDATION_RETRIES} attempts"
                    )
                    return None
    ...
    def _extract_suburb_from_geocode(self, feature: Dict) -> Optional[str]:
        ...
        for i, part in enumerate(parts):
            ...
            suburb_candidate = suburb_candidate.replace(" SA", "").replace(
                " SOUTH AUSTRALIA", ""
            )
            if suburb_candidate:
    ...
    def _find_suburb_info(self, suburb_name: str) -> Optional[Dict[str, Any]]:
        ...
        match = self.suburbs_data[
            self.suburbs_data["Suburb"].str.upper() == suburb_upper
        ]
    ...
    def generate_random_address(
        ...
        distribution_value: Optional[str] = None,
    ) -> Dict[str, Any]:
        """
        ...
        filtered_suburbs = self._filter_suburbs_by_distribution(
            distribution_type, distribution_value
        )
        ...
        # Build full address
        full_address = (
            f"{street_address}, {suburb_info['Suburb']} SA {suburb_info['Postcode']}"
        )
    ...
    def _filter_suburbs_by_distribution(
        self, distribution_type: str, distribution_value: Optional[str]
    ) -> pd.DataFrame:
        """
        ...
        if distribution_type == "suburb":
            ...
            filtered = self.suburbs_data[
                self.suburbs_data["Remoteness Level"].str.upper()
                == distribution_value.upper()
            ]
            return cast(pd.DataFrame, filtered.copy())
    ...
    def _get_suburb_coordinates(
        self, suburb_name: str
        ...
        params = {"access_token": self.mapbox_api_key, "limit": 1, "types": "place"}
    ...
    def get_available_options(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "remoteness_levels": sorted(
                self.suburbs_data["Remoteness Level"].unique().tolist()
            ),
            "socioeconomic_levels": sorted(
                self.suburbs_data["SocioEconomicStatus"].unique().tolist()
            ),
        }
