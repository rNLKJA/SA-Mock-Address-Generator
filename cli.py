...
def lookup_address(args) -> None:
    ...
    if not lookup.mapbox_api_key:
        print(
            "Error: No Mapbox API key found. Address lookup requires a valid API key.",
            file=sys.stderr,
        )
        print(
            "Please set MAPBOX_API_KEY in your environment or .env file.",
            file=sys.stderr,
        )
        sys.exit(1)
...
Examples:
  ...
  %(prog)s generate 5 --format csv             # Output as CSV
    ...
    generate_parser = subparsers.add_parser(
        "generate", help="Generate random SA addresses"
    )
    generate_parser.add_argument(
        "count", type=int, help="Number of addresses to generate"
    )
    ...
    distribution_group.add_argument(
        "--suburb", help="Generate addresses from specific suburb"
    )
    distribution_group.add_argument(
        "--council", help="Generate addresses from specific council"
    )
    distribution_group.add_argument(
        "--remoteness", help="Generate addresses from specific remoteness level"
    )
    distribution_group.add_argument(
        "--socioeconomic",
        type=int,
    ...
    # Lookup command
    lookup_parser = subparsers.add_parser("lookup", help="Look up a specific address")
    lookup_parser.add_argument(
        "address", help='Address to look up (e.g., "North Terrace, Adelaide, SA")'
    )

    # Options command
    options_parser = subparsers.add_parser(
        "options", help="Show available distribution options"
    )
